  ...
  describe GetJenkinsProjects do
    ...
    context 'when fetching master project matching request details' do
      ...
      it 'finds project matching details and master branch' do
        expect(not_matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.any_branch_pattern, details.full_branch_reference).and_return(true)
        expect(not_matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.master_branch, details.full_branch_reference, true).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.any_branch_pattern, details.full_branch_reference).and_return(true)
        expect(matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.master_branch, details.full_branch_reference, true).and_return(true)
      ...
      it 'finds first projects matching details and any non master branch' do
        ...
        expect(not_matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.master_branch, details.full_branch_reference, true).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.any_branch_pattern, details.full_branch_reference).and_return(true)
        expect(matching_project).to receive(:matches?).with(details.repository_uri, GitlabWebHookRootActionDescriptor.new.master_branch, details.full_branch_reference, true).and_return(false)
